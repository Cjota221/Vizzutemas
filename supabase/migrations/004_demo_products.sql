-- =====================================================
-- MIGRATION 004: Demo Products e Product Templates
-- =====================================================
-- Execute esta migration APÓS a 001_initial.sql

-- Tabela de produtos de demonstração para cada tema
CREATE TABLE IF NOT EXISTS demo_products (
  ...
  -- Categorização
  category VARCHAR(100), -- Ex: "Rasteiras", "Tênis", "Eletrônicos"
  badge VARCHAR(50), -- "destaque", "promocao", "novo", null
  
  -- Parcelamento
  installments INT DEFAULT 12,
  
  -- Variações (JSON array)
  -- Ex: [{"size": "35/36", "sku": "RST001", "available": true}]
...
-- Adicionar foreign key separadamente (se a tabela themes existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'demo_products_theme_id_fkey'
  ) THEN
    ALTER TABLE demo_products 
    ADD CONSTRAINT demo_products_theme_id_fkey 
...
-- Tabela de nichos com produtos pré-definidos (templates)
CREATE TABLE IF NOT EXISTS product_templates (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  
  -- Nicho do template
  niche VARCHAR(100) NOT NULL, -- "calcados_femininos", "eletronicos", "moda", etc.
  niche_label VARCHAR(100) NOT NULL, -- "Calçados Femininos", "Eletrônicos", etc.
  
  -- Info do produto template
  name VARCHAR(255) NOT NULL,
  price DECIMAL(10,2) NOT NULL,
...
-- Inserir alguns produtos template de exemplo (Calçados Femininos)
...
-- Inserir produtos template (Eletrônicos)
...
-- Inserir produtos template (Moda Feminina)
INSERT INTO product_templates (niche, niche_label, name, price, original_price, image_url, category, badge, installments, variations) VALUES
...
-- Inserir produtos template (Infantil)
INSERT INTO product_templates (niche, niche_label, name, price, image_url, category, badge, installments, variations) VALUES
